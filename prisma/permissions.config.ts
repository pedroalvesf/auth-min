/**
 * Configuração Centralizada de Permissões e Roles
 * 
 * Este arquivo define todas as permissões e roles do sistema.
 * Use `npm run db:seed` para popular o banco com essas configurações.
 */
...
export function getPermissionBySlug(slug: string): PermissionConfig | undefined {
  ...
  for (const role of ROLES) {
    ...
    for (const assignableRole of role.assignableRoles) {
      if (!ROLES.some(r => r.slug === assignableRole)) {
        errors.push(`Role "${role.slug}" references non-existent assignable role "${assignableRole}"`);
      }
    }
  }
...
if (!validation.valid) {
  console.error('⚠️  Erro na configuração de roles/permissions:');
