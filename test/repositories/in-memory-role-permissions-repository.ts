...
export class InMemoryRolePermissionsRepository
  implements RolePermissionsRepository
{
  ...
  async save(rolePermission: RolePermission): Promise<void> {
    const itemIndex = this.items.findIndex((item) =>
      item.id.equals(rolePermission.id)
    );
    if (itemIndex >= 0) {
  ...
  async findByPermissionId(permissionId: string): Promise<RolePermission[]> {
    return this.items.filter(
      (item) => item.permissionId.toString() === permissionId
    );
  }
  ...
  async findByRoleAndPermission(
    ...
    const rolePermission = this.items.find(
      (item) =>
        item.roleId.toString() === roleId &&
        item.permissionId.toString() === permissionId
    );
    return rolePermission || null;
  }
