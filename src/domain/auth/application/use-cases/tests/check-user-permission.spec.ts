...
import { setupUserRoleRelationship, addRoleToUserRepository } from "@/test/helpers/setup-user-role-permission";
...
describe("Check User Permission", () => {
  beforeEach(() => {
    ...
    sut = new CheckUserPermissionUseCase(usersRepository, permissionsRepository);
  });
  ...
  it("should return true when user has the required permission", async () => {
    ...
    jest.spyOn(permissionsRepository, 'findByRoleId').mockResolvedValue([readUsersPermission]);
  ...
  it("should return false when user does not have the required permission", async () => {
    ...
    jest.spyOn(permissionsRepository, 'findByRoleId').mockResolvedValue([writeUsersPermission]);
  ...
  it("should handle user with multiple roles and permissions", async () => {
    ...
    await usersRepository.assignRole(user.id.toString(), editorRole.id.toString());
    ...
    jest.spyOn(permissionsRepository, 'findByRoleId')
